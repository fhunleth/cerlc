...
* Implemented in native Erlang. No NIFs or dependencies required.
* Select from over 50 predefined CRC algorithms. See src/cerlc.erl for complete list
* OR Generate custom CRC algorithms by manually specifying the parameters bit width, polynomial, initial value, final XOR value, and data reflected or not.
...
Generate HTML docs locally with:
```sh
rebar3 ex_doc
```
The output is written to `_build/default/lib/cerlc/doc/index.html`.
